 ...
 * Repository: https://gitlab.com/foundrynet/coldam5e
 * Issue Tracker: https://gitlab.com/foundrynet/coldam5e/issues
 */

// Import Modules
import { COLDAM5E } from "./module/config.js";
...
Hooks.once("init", function() {
  ...
  // This will be removed when coldam5e minimum core version is updated to v9.
  if ( !game.coldam5e.isV9 ) dice.shimIsDeterministic();

  // Record Configuration Values
  CONFIG.COLDAM5E = COLDAM5E;
  ...
  Actors.registerSheet("coldam5e", ActorSheet5eCharacter, {
    types: ["character"],
    makeDefault: true,
    label: "COLDAM5E.SheetClassCharacter"
  });
  Actors.registerSheet("coldam5e", ActorSheet5eNPC, {
    types: ["npc"],
    makeDefault: true,
    label: "COLDAM5E.SheetClassNPC"
  });
  Actors.registerSheet("coldam5e", ActorSheet5eVehicle, {
    types: ["vehicle"],
    makeDefault: true,
    label: "COLDAM5E.SheetClassVehicle"
  });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("coldam5e", ItemSheet5e, {
    makeDefault: true,
    label: "COLDAM5E.SheetClassItem"
  });
...
Hooks.once("setup", function() {
  ...
  preLocalizeConfig(CONFIG.COLDAM5E, localizeKeys, sortKeys);
  CONFIG.COLDAM5E.trackableAttributes = expandAttributeList(CONFIG.COLDAM5E.trackableAttributes);
  CONFIG.COLDAM5E.consumableResources = expandAttributeList(CONFIG.COLDAM5E.consumableResources);
});
...
Hooks.once("ready", function() {
  ...
  const currentVersion = game.settings.get("coldam5e", "systemMigrationVersion");
  const NEEDS_MIGRATION_VERSION = "1.5.6";
  const COMPATIBLE_MIGRATION_VERSION = 0.80;
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;
  if ( !currentVersion && totalDocuments === 0 ) return game.settings.set("coldam5e", "systemMigrationVersion", game.system.data.version);
...
Hooks.on("canvasInit", function() {
  ...
  canvas.grid.diagonalRule = game.settings.get("coldam5e", "diagonalMovement");
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("coldam5e", "autoCollapseItemCards")) html.find(".card-content").hide();
});
