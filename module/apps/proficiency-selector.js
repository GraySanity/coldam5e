...
export default class ProficiencySelector extends TraitSelector {
  ...
  static async getChoices(type, chosen=[]) {
    let data = Object.entries(CONFIG.COLDAM5E[`${type}Proficiencies`]).reduce((obj, [key, label]) => {
      obj[key] = { label: label, chosen: chosen.includes(key) };
      return obj;
    }, {});

    const ids = CONFIG.COLDAM5E[`${type}Ids`];
    const map = CONFIG.COLDAM5E[`${type}ProficienciesMap`];
    if ( ids !== undefined ) {
    ...
    if ( type === "tool" ) {
      data.vehicle.children = Object.entries(CONFIG.COLDAM5E.vehicleTypes).reduce((obj, [key, label]) => {
        obj[key] = { label: label, chosen: chosen.includes(key) };
        return obj;
      }, {});
  ...
  static getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
    let pack = CONFIG.COLDAM5E.sourcePacks.ITEMS;
