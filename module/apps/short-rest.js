...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      template: "systems/coldam5e/templates/apps/short-rest.html",
      classes: ["coldam5e", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("coldam5e", "restVariant");
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("COLDAM5E.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("COLDAM5E.Rest"),
            callback: html => {
              let newDay = false;
              if (game.settings.get("coldam5e", "restVariant") === "gritty") {
