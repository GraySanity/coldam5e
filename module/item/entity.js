...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.COLDAM5E.areaTargetTypes);
  }
   ...
   */
  get isArmor() {
    return this.data.data.armor?.type in CONFIG.COLDAM5E.armorTypes;
  }
  ...
  get areEffectsSuppressed() {
    ...
    return this.data.data.attunement === CONFIG.COLDAM5E.attunementTypes.REQUIRED;
  }
  ...
  prepareDerivedData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("COLDAM5E.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("COLDAM5E.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "COLDAM5E.Attack" : "COLDAM5E.Action");
      else labels.featType = game.i18n.localize("COLDAM5E.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("COLDAM5E.AC")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("COLDAM5E.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  prepareFinalAttributes() {
    ...
    if ( this.data.data.hasOwnProperty("actionType") ) {
      // Ability checks
      this.labels.abilityCheck = game.i18n.format("COLDAM5E.AbilityPromptTitle", {
        ability: CONFIG.COLDAM5E.abilities[this.data.data?.ability]
      });
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.COLDAM5E.abilities[save.ability];
    this.labels.save = game.i18n.format("COLDAM5E.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  getCriticalThreshold() {
    const itemData = this.data.data;
    const actorFlags = this.actor.data.flags.coldam5e || {};
    if ( !this.hasAttack || !itemData ) return;
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    const requireSpellSlot = isSpell && (id.level > 0) && CONFIG.COLDAM5E.spellUpcastModes.includes(id.preparation.mode);
    ...
    if ( createMeasuredTemplate ) {
      const template = game.coldam5e.canvas.AbilityTemplate.fromItem(item);
  ...
  _getUsageUpdates({consumeQuantity, consumeRecharge, consumeResource, consumeSpellLevel, consumeUsage}) {
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("COLDAM5E.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellLevel === "pact" ? "COLDAM5E.SpellProgPact" : `COLDAM5E.SpellLevel${id.level}`);
        ui.notifications.warn(game.i18n.format("COLDAM5E.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.COLDAM5E.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("COLDAM5E.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( resource === undefined ) {
      ui.notifications.warn(game.i18n.format("COLDAM5E.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("COLDAM5E.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/coldam5e/templates/chat/item-card.html", templateData);
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["coldam5e.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.COLDAM5E.attunementTypes.REQUIRED ) {
        props.push(game.i18n.localize(CONFIG.COLDAM5E.attunements[CONFIG.COLDAM5E.attunementTypes.REQUIRED]));
      }
      props.push(
        game.i18n.localize(data.equipped ? "COLDAM5E.Equipped" : "COLDAM5E.Unequipped"),
        game.i18n.localize(data.proficient ? "COLDAM5E.Proficient" : "COLDAM5E.NotProficient")
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.COLDAM5E.equipmentTypes[data.armor.type],
      labels.armor || null,
      data.stealth.value ? game.i18n.localize("COLDAM5E.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.COLDAM5E.weaponTypes[data.weaponType]
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.COLDAM5E.consumableTypes[data.consumableType],
      `${data.uses.value}/${data.uses.max} ${game.i18n.localize("COLDAM5E.Charges")}`
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.COLDAM5E.abilities[data.ability] || null,
      CONFIG.COLDAM5E.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("COLDAM5E.ItemTypeLoot"),
      data.weight ? `${data.weight} ${game.i18n.localize("COLDAM5E.AbbreviationLbs")}` : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const flags = this.actor.data.flags.coldam5e || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }
    let title = `${this.name} - ${game.i18n.localize("COLDAM5E.AttackRoll")}`;
    ...
    let rollConfig = {
      ...
      messageData: {
        "flags.coldam5e.roll": {type: "attack", itemId: this.id },
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const actorData = this.actor.data.data;
    const messageData = {
      "flags.coldam5e.roll": {type: "damage", itemId: this.id},
      speaker: ChatMessage.getSpeaker({actor: this.actor})
    };
    ...
    const actionFlavor = game.i18n.localize(itemData.actionType === "heal" ? "COLDAM5E.Healing" : "COLDAM5E.DamageRoll");
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.coldam5e.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("coldam5e", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula({spellLevel}={}) {
    ...
    const rollData = this.getRollData();
    if ( spellLevel ) rollData.item.level = spellLevel;
    const title = `${this.name} - ${game.i18n.localize("COLDAM5E.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.coldam5e.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("COLDAM5E.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "COLDAM5E.ItemRechargeSuccess" : "COLDAM5E.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const parts = ["@mod"];
    const title = `${this.name} - ${game.i18n.localize("COLDAM5E.ToolCheck")}`;
    ...
    const rollConfig = mergeObject({
      ...
      chooseModifier: true,
      halflingLucky: this.actor.getFlag("coldam5e", "halflingLucky" ) || false,
      reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("coldam5e", "reliableTalent"),
      messageData: {"flags.coldam5e.roll": {type: "tool", itemId: this.id }}
    }, options);
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("coldam5e", "itemData");
    const item = storedData ? new this(storedData, {parent: actor}) : actor.items.get(card.dataset.itemId);
    if ( !item ) {
      return ui.notifications.error(game.i18n.format("COLDAM5E.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}));
    }
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("COLDAM5E.ActionWarningNoToken"));
    return targets;
  }
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.COLDAM5E.armorProficienciesMap[this.data.data.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    const weaponProf = CONFIG.COLDAM5E.weaponProficienciesMap[this.data.data.weaponType];
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.COLDAM5E.sourcePacks.ITEMS}.${CONFIG.COLDAM5E.spellScrollIds[level]}`;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("COLDAM5E.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
