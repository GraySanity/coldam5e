...
export default class ItemSheet5e extends ItemSheet {
  ...
  /** @inheritdoc */
  get template() {
    const path = "systems/coldam5e/templates/items/";
  ...
  async _getItemBaseTypes(item) {
    ...
    const ids = CONFIG.COLDAM5E[`${type}Ids`];
      ...
      return actor.items.reduce((obj, i) => {
        ...
        if ( uses.per && uses.max ) {
          const label = uses.per === "charges"
            ? ` (${game.i18n.format("COLDAM5E.AbilityUseChargesLabel", {value: uses.value})})`
            : ` (${game.i18n.format("COLDAM5E.AbilityUseConsumableLabel", {max: uses.max, per: uses.per})})`;
          obj[i.id] = i.name + label;
        }

        // Recharging items
        const recharge = i.data.data.recharge || {};
        if ( recharge.value ) obj[i.id] = `${i.name} (${game.i18n.format("COLDAM5E.Recharge")})`;
        return obj;
      }, {});
    }
    else return {};
  ...
  _getItemStatus(item) {
    if ( item.type === "spell" ) {
      return CONFIG.COLDAM5E.spellPreparationModes[item.data.preparation];
    }
    else if ( ["weapon", "equipment"].includes(item.type) ) {
      return game.i18n.localize(item.data.equipped ? "COLDAM5E.Equipped" : "COLDAM5E.Unequipped");
    }
    else if ( item.type === "tool" ) {
      return game.i18n.localize(item.data.proficient ? "COLDAM5E.Proficient" : "COLDAM5E.NotProficient");
    }
  }
  ...
  _getItemProperties(item) {
    ...
    if ( item.type === "weapon" ) {
      props.push(...Object.entries(item.data.properties)
        .filter(e => e[1] === true)
        .map(e => CONFIG.COLDAM5E.weaponProperties[e[0]]));
    }
      ...
      props.push(
        labels.components,
        labels.materials,
        item.data.components.concentration ? game.i18n.localize("COLDAM5E.Concentration") : null,
        item.data.components.ritual ? game.i18n.localize("COLDAM5E.Ritual") : null
      );
    }

    else if ( item.type === "equipment" ) {
      props.push(CONFIG.COLDAM5E.equipmentTypes[item.data.armor.type]);
    ...
    if ( item.data.actionType ) {
      props.push(CONFIG.COLDAM5E.itemActionTypes[item.data.actionType]);
    }
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( data.data?.uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        ui.notifications.error(game.i18n.format("COLDAM5E.FormulaCannotContainDiceWarn", {
          name: game.i18n.localize("COLDAM5E.LimitedUses")
        }));
      }
    }
  ...
  _onConfigureTraits(event) {
    ...
    switch (a.dataset.options) {
      case "saves":
        options.choices = CONFIG.COLDAM5E.abilities;
        options.valueKey = null;
        break;
      case "skills.choices":
        options.choices = CONFIG.COLDAM5E.skills;
        options.valueKey = null;
        break;
      case "skills":
        const skills = this.item.data.data.skills;
        const choiceSet = skills.choices?.length ? skills.choices : Object.keys(CONFIG.COLDAM5E.skills);
        options.choices =
          Object.fromEntries(Object.entries(CONFIG.COLDAM5E.skills).filter(([skill]) => choiceSet.includes(skill)));
