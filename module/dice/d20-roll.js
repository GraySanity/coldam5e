...
export default class D20Roll extends Roll {
   ...
   * @type {string}
   */
  static EVALUATION_TEMPLATE = "systems/coldam5e/templates/chat/roll-dialog.html";
  ...
  async toMessage(messageData={}, options={}) {
    ...
    // Add appropriate advantage mode message flavor and coldam5e roll flags
    messageData.flavor = messageData.flavor || this.options.flavor;
    if ( this.hasAdvantage ) messageData.flavor += ` (${game.i18n.localize("COLDAM5E.Advantage")})`;
    else if ( this.hasDisadvantage ) messageData.flavor += ` (${game.i18n.localize("COLDAM5E.Disadvantage")})`;

    // Add reliable talent to the d20-term flavor text if it applied
    if ( this.options.reliableTalent ) {
      const d20 = this.dice[0];
      const isRT = d20.results.every(r => !r.active || (r.result < 10));
      const label = `(${game.i18n.localize("COLDAM5E.FlagsReliableTalent")})`;
  ...
  async configureDialog({title, defaultRollMode, defaultAction=D20Roll.ADV_MODE.NORMAL, chooseModifier=false,
    ...
    const content = await renderTemplate(template ?? this.constructor.EVALUATION_TEMPLATE, {
      ...
      chooseModifier,
      defaultAbility,
      abilities: CONFIG.COLDAM5E.abilities
    });
    ...
    return new Promise(resolve => {
      new Dialog({
        ...
        buttons: {
          advantage: {
            label: game.i18n.localize("COLDAM5E.Advantage"),
            callback: html => resolve(this._onDialogSubmit(html, D20Roll.ADV_MODE.ADVANTAGE))
          },
          normal: {
            label: game.i18n.localize("COLDAM5E.Normal"),
            callback: html => resolve(this._onDialogSubmit(html, D20Roll.ADV_MODE.NORMAL))
          },
          disadvantage: {
            label: game.i18n.localize("COLDAM5E.Disadvantage"),
  ...
  _onDialogSubmit(html, advantageMode) {
    ...
    if ( form.ability?.value ) {
      ...
      this.options.flavor += ` (${CONFIG.COLDAM5E.abilities[form.ability.value]})`;
    }
