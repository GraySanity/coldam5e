...
import {COLDAM5E} from "../config.js";
...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("coldam5e", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.coldam5e || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag("coldam5e", "transformOptions");
      const original = game.actors?.get(this.getFlag("coldam5e", "originalActor"));
      if (original) {
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.COLDAM5E.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.COLDAM5E.CR_EXP_LEVELS[cr];
  }
  ...
  async addEmbeddedItems(items, prompt=true) {
    ...
    if (prompt) {
      const itemIdsToAdd = await SelectItemsPrompt.create(items, {
        hint: game.i18n.localize("COLDAM5E.AddEmbeddedItemPromptHint")
      });
  ...
  static async loadClassFeatures({className="", subclassName="", level=1, priorLevel=0}={}) {
    ...
    const clsConfig = CONFIG.COLDAM5E.classFeatures[className];
    if (!clsConfig) return [];
  ...
  _prepareSkills(actorData, bonusData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = COLDAM5E.characterFlags;
    const joat = flags.jackOfAllTrades;
  ...
  _computeSpellcastingProgression(actorData) {
    ...
    const slots = COLDAM5E.SPELL_SLOT_TABLE[levels - 1] || [];
  ...
  _computeArmorClass(data) {
    ...
    ac.warnings = [];
    let cfg = CONFIG.COLDAM5E.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.COLDAM5E.armorClasses.flat;
    }

    // Identify Equipped Items
    const armorTypes = new Set(Object.keys(CONFIG.COLDAM5E.armorTypes));
    ...
    switch ( ac.calc ) {
      ...
      case "default":
        if ( armors.length ) {
          if ( armors.length > 1 ) ac.warnings.push("COLDAM5E.WarnMultipleArmor");
      ...
      default:
        ...
        try {
          ...
          ac.warnings.push("COLDAM5E.WarnBadACFormula");
          const replaced = Roll.replaceFormulaData(CONFIG.COLDAM5E.armorClasses.default.formula, rollData);
          ac.base = Roll.safeEval(replaced);
        }
        break;
    }
    ...
    // Equipped Shield
    if ( shields.length ) {
      if ( shields.length > 1 ) ac.warnings.push("COLDAM5E.WarnMultipleShields");
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("coldam5e", "currencyWeight") && actorData.data.currency ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);

      const currencyPerWeight = game.settings.get("coldam5e", "metricWeightUnits")
        ? CONFIG.COLDAM5E.encumbrance.currencyPerWeight.metric
        : CONFIG.COLDAM5E.encumbrance.currencyPerWeight.imperial;
    ...
    if ( this.getFlag("coldam5e", "powerfulBuild") ) mod = Math.min(mod * 2, 8);

    // Compute Encumbrance percentage
    weight = weight.toNearest(0.1);

    const strengthMultiplier = game.settings.get("coldam5e", "metricWeightUnits")
      ? CONFIG.COLDAM5E.encumbrance.strMultiplier.metric
      : CONFIG.COLDAM5E.encumbrance.strMultiplier.imperial;
  ...
  async _preCreate(data, options, user) {
    ...
    // Token size category
    const s = CONFIG.COLDAM5E.tokenSizes[this.data.data.traits.size || "med"];
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( newSize && (newSize !== foundry.utils.getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.COLDAM5E.tokenSizes[newSize];
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("coldam5e", "remarkableAthlete") && COLDAM5E.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("coldam5e", "reliableTalent"));

    // Roll and return
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.format("COLDAM5E.SkillPromptTitle", {skill: CONFIG.COLDAM5E.skills[skillId]})}: ${this.name}`,
      halflingLucky: this.getFlag("coldam5e", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.coldam5e.roll": {type: "skill", skillId }
      }
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.COLDAM5E.abilities[abilityId];
    new Dialog({
      title: `${game.i18n.format("COLDAM5E.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("COLDAM5E.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("COLDAM5E.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("COLDAM5E.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.coldam5e.roll": {type: "ability", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.format("COLDAM5E.SavePromptTitle", {ability: label})}: ${this.name}`,
      halflingLucky: this.getFlag("coldam5e", "halflingLucky"),
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.coldam5e.roll": {type: "save", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("COLDAM5E.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("coldam5e", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.localize("COLDAM5E.DeathSavingThrow")}: ${this.name}`,
      halflingLucky: this.getFlag("coldam5e", "halflingLucky"),
      targetValue: 10,
      messageData: {
        speaker: speaker,
        "flags.coldam5e.roll": {type: "death"}
      }
    });
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        chatString = "COLDAM5E.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        chatString = "COLDAM5E.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        chatString = "COLDAM5E.DeathSaveFailure";
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("COLDAM5E.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const title = `${game.i18n.localize("COLDAM5E.HitDiceRoll")}: ${this.name}`;
    ...
    const roll = await damageRoll({
      ...
      messageData: {
        ...
        "flags.coldam5e.roll": {type: "hitDie"}
      }
    });
    if ( !roll ) return null;
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    switch (game.settings.get("coldam5e", "restVariant")) {
      case "normal": restFlavor = (longRest && newDay) ? "COLDAM5E.LongRestOvernight" : `COLDAM5E.${length}RestNormal`; break;
      case "gritty": restFlavor = (!longRest && newDay) ? "COLDAM5E.ShortRestOvernight" : `COLDAM5E.${length}RestGritty`; break;
      case "epic": restFlavor = `COLDAM5E.${length}RestEpic`; break;
    }

    // Determine the chat message to display
    if ( diceRestored && healthRestored ) message = `COLDAM5E.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "COLDAM5E.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "COLDAM5E.LongRestResultHitDice";
    else message = `COLDAM5E.${length}RestResultShort`;
  ...
  convertCurrency() {
    ...
    const conversion = Object.entries(CONFIG.COLDAM5E.currencies);
    conversion.reverse();
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("coldam5e", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("COLDAM5E.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = this.toJSON();
    o.flags.coldam5e = o.flags.coldam5e || {};
    o.flags.coldam5e.transformOptions = {mergeSkills, mergeSaves};
    ...
    if (!keepClass && d.data.details.cr) {
      d.items.push({
        type: "class",
        name: game.i18n.localize("COLDAM5E.PolymorphTmpClass"),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.coldam5e.originalActor ) d.flags.coldam5e.originalActor = this.id;
    d.flags.coldam5e.isPolymorphed = true;
    ...
    Hooks.callAll("coldam5e.transformActor", this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.isOwner ) {
      return ui.notifications.warn(game.i18n.localize("COLDAM5E.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    ...
    entryOptions.push({
      name: "COLDAM5E.PolymorphRestoreTransformation",
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.COLDAM5E.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("COLDAM5E.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.COLDAM5E.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
  ...
  static prepareProficiencies(data, type) {
    const profs = CONFIG.COLDAM5E[`${type}Proficiencies`];
    const itemTypes = CONFIG.COLDAM5E[`${type}Ids`];
    ...
    for ( const key of values ) {
      ...
      } else if ( type === "tool" && CONFIG.COLDAM5E.vehicleTypes[key] ) {
        data.selected[key] = CONFIG.COLDAM5E.vehicleTypes[key];
      }
    }
  ...
  _displayScrollingDamage(dhp) {
    ...
    for ( let t of tokens ) {
      ...
      t.hud.createScrollingText(dhp.signedString(), {
        ...
        fill: CONFIG.COLDAM5E.tokenHPColors[dhp < 0 ? "damage" : "healing"],
        stroke: 0x000000,
        strokeThickness: 4,
        jitter: 0.25
      });
   ...
   * @param {string} ability  Ability key as defined in `CONFIG.COLDAM5E.abilities`.
   * @returns {number}        Spell save DC for provided ability.
   * @deprecated since coldam5e 0.97
   */
  getSpellDC(ability) {
   ...
   * @deprecated since coldam5e 1.2.0
   */
