...
export default class ActorSheet5e extends ActorSheet {
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/coldam5e/templates/actors/limited-sheet.html";
    return `systems/coldam5e/templates/actors/${this.actor.data.type}-sheet.html`;
  }
  ...
  getData(options) {
    ...
    // Currency Labels
    data.labels.currencies = Object.entries(CONFIG.COLDAM5E.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});

    // Proficiency
    if ( game.settings.get("coldam5e", "proficiencyModifier") === "dice" ) {
    ...
    for ( let [a, abl] of Object.entries(actorData.data.abilities)) {
      ...
      abl.hover = CONFIG.COLDAM5E.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.COLDAM5E.abilities[a];
    ...
    if ( actorData.data.skills ) {
      for (let [s, skl] of Object.entries(actorData.data.skills)) {
        skl.ability = CONFIG.COLDAM5E.abilityAbbreviations[skl.ability];
        skl.icon = this._getProficiencyIcon(skl.value);
        skl.hover = CONFIG.COLDAM5E.proficiencyLevels[skl.value];
        skl.label = CONFIG.COLDAM5E.skills[s];
  ...
  _getMovementSpeed(actorData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("COLDAM5E.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("COLDAM5E.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("COLDAM5E.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("COLDAM5E.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("COLDAM5E.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("COLDAM5E.MovementWalk")} ${movement.walk}`]);
    }
  ...
  _getSenses(actorData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.COLDAM5E.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
  ...
  _prepareArmorClassAttribution(data) {
    ...
    const cfg = CONFIG.COLDAM5E.armorClasses[ac.calc];
    const attribution = [];
    ...
    switch ( ac.calc ) {
      ...
      case "flat":
        return [{
          label: game.i18n.localize("COLDAM5E.ArmorClassFlat"),
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("COLDAM5E.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      case "default":
        ...
        attribution.push({
          label: hasArmor ? this.actor.armor.name : game.i18n.localize("COLDAM5E.ArmorClassUnarmored"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: hasArmor ? this.actor.armor.data.data.armor.value : 10
        });
        if ( ac.dex !== 0 ) {
          attribution.push({
            label: game.i18n.localize("COLDAM5E.AbilityDex"),
      ...
      default:
        ...
        attribution.unshift({
          label: game.i18n.localize("COLDAM5E.PropertyBase"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
        });
        break;
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.actor.shield?.name ?? game.i18n.localize("COLDAM5E.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
    ...
    if ( ac.cover !== 0 ) attribution.push({
      label: game.i18n.localize("COLDAM5E.Cover"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.cover
    });
    return attribution;
  ...
  _prepareTraits(traits) {
    const map = {
      dr: CONFIG.COLDAM5E.damageResistanceTypes,
      di: CONFIG.COLDAM5E.damageResistanceTypes,
      dv: CONFIG.COLDAM5E.damageResistanceTypes,
      ci: CONFIG.COLDAM5E.conditionTypes,
      languages: CONFIG.COLDAM5E.languages
    };
  ...
  _prepareSpellbook(data, spells) {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.COLDAM5E.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.COLDAM5E.spellLevels[lvl], levels[sl]);
      }
    }

    // Pact magic users have cantrips and a pact magic section
    if ( levels.pact && levels.pact.max ) {
      if ( !spellbook["0"] ) registerSection("spell0", 0, CONFIG.COLDAM5E.spellLevels[0]);
      const l = levels.pact;
      const config = CONFIG.COLDAM5E.spellPreparationModes.pact;
      const level = game.i18n.localize(`COLDAM5E.SpellLevel${levels.pact.level}`);
    ...
    spells.forEach(spell => {
      ...
      if ( mode in sections ) {
        ...
        if ( !spellbook[s] ) {
          const l = levels[mode] || {};
          const config = CONFIG.COLDAM5E.spellPreparationModes[mode];
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.COLDAM5E.spellLevels[s], {levels: levels[sl]});
      }
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.COLDAM5E.proficiencyLevels`.
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("coldam5e", "allowPolymorphing"));
    ...
    const rememberOptions = html => {
      ...
      const settings = mergeObject(game.settings.get("coldam5e", "polymorphSettings") || {}, options);
      game.settings.set("coldam5e", "polymorphSettings", settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize("COLDAM5E.PolymorphPromptTitle"),
      content: {
        options: game.settings.get("coldam5e", "polymorphSettings"),
        i18n: COLDAM5E.polymorphSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize("COLDAM5E.PolymorphAcceptSettings"),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: '<i class="fas fa-paw"></i>',
          label: game.i18n.localize("COLDAM5E.PolymorphWildShape"),
        ...
        polymorph: {
          ...
          label: game.i18n.localize("COLDAM5E.Polymorph"),
          callback: html => this.actor.transformInto(sourceActor, {
            transformTokens: rememberOptions(html).transformTokens
          })
        },
      ...
      classes: ["dialog", "coldam5e"],
      width: 600,
      template: "systems/coldam5e/templates/apps/polymorph-prompt.html"
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      return ui.notifications.warn(game.i18n.format("COLDAM5E.ActorWarningInvalidItem", {
    ...
    if ( itemData.data ) {
      ...
      itemData.data.attunement = Math.min(itemData.data.attunement, CONFIG.COLDAM5E.attunementTypes.REQUIRED);
    }
  ...
  _onItemCreate(event) {
    ...
    const itemData = {
      name: game.i18n.format("COLDAM5E.ItemNew", {type: game.i18n.localize(`COLDAM5E.ItemType${type.capitalize()}`)}),
      type: type,
  ...
  _onTraitSelector(event) {
    ...
    const a = event.currentTarget;
    const label = a.parentElement.querySelector("label");
    const choices = CONFIG.COLDAM5E[a.dataset.options];
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: "COLDAM5E.PolymorphRestoreTransformation",
